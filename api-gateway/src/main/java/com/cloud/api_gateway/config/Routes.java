...
public class Routes {
        ...
        public RouterFunction<ServerResponse> accountServiceRoute() {
                return GatewayRouterFunctions.route("account_service")
                                .route(RequestPredicates.path("/api/v1/accounts/**"),
                                                HandlerFunctions.http(accountServiceUrl))
                                .filter(authFilter)
        ...
        public RouterFunction<ServerResponse> transactionServiceRoute() {
                return GatewayRouterFunctions.route("transaction_service")
                                .route(RequestPredicates.path("/api/v1/transactions/**"),
                                                HandlerFunctions.http(transactionServiceUrl))
                                .filter(authFilter)
        ...
        public RouterFunction<ServerResponse> auditServiceRoute() {
                return GatewayRouterFunctions.route("audit_service")
                                .route(RequestPredicates.path("/api/v1/audit/**"),
                                                HandlerFunctions.http(auditServiceUrl))
                                .filter(authFilter)
        ...
        public RouterFunction<ServerResponse> fallbackRoute() {
                return route("fallbackRoute")
                                .route(RequestPredicates.path("/fallbackRoute"), request -> ServerResponse
